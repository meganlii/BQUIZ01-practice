      ...
      <tbody>
        <tr class="yel">
          ...
          <td width="50%">
            ...
            1. 複製31行 貼到後台跟前台主頁-100行backend.php、index.php
            <?=$Total->find(1)['total'] ;?>
            2. 在網頁html程式碼裡面塞入php程式碼/變數  故稱義大利麵條
            其他程式也有類似寫法asp jsp php
            因為 HTML 和 PHP 混在一起，就像煮過頭的義大利麵條一樣，糾纏在一起，難以分離

            3. 進站總人數及頁尾版權顯示的兩段程式碼 獨立寫兩個頁面，超過三個以上，就建議另外切出檔案再include到需要的版面
            要個別貼到兩個頁面，如果頁面多10幾個不好維護更新，即便只是幾個字而已也要切出來
            */
            $row = $Total->find(1);
            ?>

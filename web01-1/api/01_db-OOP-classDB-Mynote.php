...
function q($sql)
{
    ...
    4. 回傳值形式
    回傳值的內容 會因為傳遞時SQL語法而有不同的結果，
    有時是陣列(關聯/數字索引)，有時是單一個值
    
    FETCH_ASSOC
    回傳 帶名稱的陣列
    返回 關聯陣列(二維)key=value，不返回 數字/索引陣列
    ...
    fetchColumn($n)
    回傳 單一個值  返回該筆資料中指定欄位的資料，$n為欄位的索引值(0,1,2…)
    只返回第一列的第一個欄位值
...
class DB
{
    ...
    * $id 一定存在，因為是必要參數
    * 只需要檢查 $id「類型」is_array，不用檢查「是否存在isset()」
    * find();           // ❌ 錯誤！缺少必要參數
    * find(1);          // ✓ 正確
    * find(['name' => 'John']); // ✓ 正確
...
2. 用法 $this->table = 
* $title = $Title->find($id)
* -> 右箭頭符號是PHP物件運算子，用來呼叫物件的方法或存取物件的屬性。
